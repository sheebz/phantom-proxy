    ...
    //creates a new phantomjs process
        createServer = function (options, callbackFn) {
                ...
                parseDataBuffer = function (dataBuffer) {
                    ...
                    switch (parsedEvent.source) {
                        ...
                        case 'server' :
                            ...
                            //check for server module failure
                            if (!parsedEvent.args[0].result) {
                                throw new Error('unable to start phantomjs server module');
                            }

                            options.debug && console.log('server created'.green);

                            //build proxy and execute callback
                            var Webpage = require('./webpage'),
                                page = page || new Webpage(options, phantomProcess),
                            ...
                            break;
                        default :
                            this.options.debug && console.log('emmiting %s event'.grey, parsedEvent.source);

                            //pass args as array w/ out source argument, then emit event on page object
            ...
            var createReadStream = function () {
                ...
                self.options.debug && console.log('reading %s'.green, options.eventStreamPath);

                var streamReader = fs.createReadStream(options.eventStreamPath, {start:streamPosition });

                streamReader.on('end', function () {
                    self.options.debug && console.log('EOF'.yellow);
            ...
            var onFileChanged = function (curr, prev) {
                options.debug && console.log('file changed');
            ...
            phantomProcess.on('close', function () {
                options.debug && console.log('close called'.green);
            });

            phantomProcess.on('disconnect', function () {
                options.debug && console.log('disconnect called'.green);
            });
            phantomProcess.on('message', function () {
                options.debug && console.log('message called'.green);
            });
    ...
    module.exports = {
        //terminates phantomjs process
            ...
            phantomProcess.on('exit', function () {
                self.options.debug && console.log('phantom process exited'.green);
                callbackFn(true);
            });
            request.post(this.options.hostAndPort + '/phantom/functions/exit', {
                ...
                function (error, response, body) {
                    self.options.debug && console.log('exit request sent. response was : %s', body);
                });
        },
