    ...
    var WebPage = function (options, phantomProcess) {
            ...
            try {
                ...
                msgArray.forEach(function (stringValue) {
                    if (stringValue && stringValue.length > 2) {
                        event = JSON.parse(stringValue.replace('#END-EVENT', ''));
                        var args = event.args;
                        args.unshift(event.source);
        ...
        _.extend(this, {
            ...
            open:function (url, callbackFn) {
                var self = this;
                request.post(self.host + '/page/functions/open', {form:{ arguments:JSON.stringify(
                        [
                            url
                        ], null, 4)}},
                    function (error, response, body) {
            ...
            render:function (filename, callbackFn) {
                var self = this;
                request.post(self.host + '/page/functions/render', {form:{ arguments:JSON.stringify(
                        [
                            filename
                        ], null, 4)}},
                    function (error, response, body) {
            ...
            renderBase64:function (format, callbackFn) {
                var self = this;
                request.post(self.host + '/page/functions/renderBase64', {form:{ arguments:JSON.stringify(
                        [
                            format
                        ], null, 4)}},
                    function (error, response, body) {
