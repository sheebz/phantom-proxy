...
(function (module) {
    ...
    module.exports = {
        initialize:function () {
            ...
            this.eventWriter = require('./eventWriter').create(this.options);

            //create wrapper for page interface
            this.pageWrapper = require('./webpage').create(this.options);
            //create wrapper for phantom interface
            this.phantomWrapper = require('./phantom').create(this.options);

            //server instance
            this.server = require('webserver').create();

            var service = this.server.listen(this.options.port, function (request, response) {
               // self.options.debug && console.log(request);
                ...
                try {
                    ...
                    console.error(error);
//                    this.eventWriter.triggerEvent('proxy', {args:{
//                        'error':error
//                    }});
//                    throw new Error(error);
            ...
            this.eventWriter.triggerEvent('server',
                {
                    "type":'serverCreated',
                    "args":
                        [
